...
import Utility as util, pywinstyles, threading

# Private util functions for input checking
...
class Task(customtkinter.CTkFrame):
    ...
    def load_task_modify_menu(self, 
        list_name: str = None, 
        task_name: str = None
    ):
        ...
        self.modify_task_frame = customtkinter.CTkFrame(self.list.main_frame,
            width=500, height=400, fg_color="#383736")
        self.modify_task_frame.place(relx=0.5, rely=0.5,
        ...
        task_date, task_time = (
            ...
            due_date and due_date['time']
        )

        # Title
        customtkinter.CTkLabel(self.modify_task_frame,
            text=task_name and "EDIT TASK" or "CREATE TASK",
            font=self.main.get_font(size=25)).pack(pady=10)

        # Close
        customtkinter.CTkButton(self.modify_task_frame, text="X", 
            width=35, corner_radius=0,
            font=self.main.get_font(size=22, bold=True), 
            fg_color="red", hover_color="dark red",
            command=lambda: self.reload_tasks_frame(self.modify_task_frame)
        ...
        task_description_var = customtkinter.StringVar(
            value=len(task_description) > 0 and task_description 
            or "Task Description"
        )

        self.task_name = customtkinter.CTkEntry(self.modify_task_frame, 
            corner_radius=0, width=400, border_width=0, 
        ...
        self.task_description = customtkinter.CTkEntry(self.modify_task_frame, 
            corner_radius=0, width=400, border_width=0, fg_color="#D9D9D9", 
            text_color=len(task_description) > 0 and "black" or "grey",   
        ...
        self.task_description.place(relx=0.5,rely=0.35,
            anchor=customtkinter.CENTER)

        priority_string_var = customtkinter.StringVar(
            value=priority_level and str(priority_level) or "1"
        )

        self.task_priority = customtkinter.CTkComboBox(self.modify_task_frame, 
            values=self.PRIORITY_LEVELS, variable=priority_string_var, 
            corner_radius=0,
        ...
        self.task_date = customtkinter.CTkLabel(self.modify_task_frame, 
            width=75, height=35, fg_color="#D9D9D9", text_color="black", 
        ...
        self.valid_time = (
            task_time or f'{current_time.strftime("%I:%M")} ' +
            f'{datetime.strptime(formatted_time, "%H:%M").strftime("%p")}'
        )
        
        self.time = customtkinter.StringVar(value=self.valid_time)
        
        self.task_time = customtkinter.CTkEntry(self.modify_task_frame, 
            width=75, height=35, fg_color="#D9D9D9",
        ...
        self.calendar = customtkinter.CTkButton(self.modify_task_frame, 
            text="", corner_radius=0,
        ...
        customtkinter.CTkLabel(self.modify_task_frame,
            ...
            text_color="grey",
            font=('Helvetica', 12, 'bold', 'italic'), justify=customtkinter.LEFT
        ).place(relx=0.37,rely=0.7,anchor=customtkinter.CENTER)

        # Button to create or save information about the task
        customtkinter.CTkButton(self.modify_task_frame,
            text=task_name and "SAVE" or "COMPLETE", text_color="white",
    ...
    def validate_time(self, event = None):
        ...
        if "AM" in time_str or "PM" in time_str:
            try:
                formatted_time = (
                    datetime.strptime(time_str, "%I:%M %p").strftime("%I:%M %p")
                )
    ...
    def load_calendar(self):
        ...
        self.task_calendar = Calendar(self.modify_task_frame, 
            selectmode='day', mindate=datetime.now(),
        ...
        self.close_calendar = customtkinter.CTkButton(self.modify_task_frame,
            text="Close Calendar", width=100, height=20,
            hover_color="red", command=destroy_calendar)
    ...
    def create_task_template(self, task_data: dict):
        ...
        task_item = customtkinter.CTkFrame(
            self.tasks_container, width=400, height=100,
            fg_color=STATE_COLOURS[task_state], corner_radius=15,
        ...
        customtkinter.CTkButton(
            task_item, text="EDIT", fg_color="#00BFFF", hover_color="#1E90FF",
            text_color="white", corner_radius=5, width=60, height=25,
            command=lambda: self.load_task_modify_menu(task_name=task_name)
        ...
        customtkinter.CTkButton(
            ...
            command=lambda: self.task_delete_activated(task_name)
        ...
        customtkinter.CTkCheckBox(
            ...
            corner_radius=5, width=10,
            variable=completed_check, command=task_completed
    ...
    def load_saved_tasks(self, list_data):
        ...
        loading_label = customtkinter.CTkLabel(self.tasks_frame, text='',
            width=100,height=100,bg_color="transparent", fg_color="transparent", 
        ...
        for level, data in sorted(prioritised_dict.items()):
            if not data:
                continue
            # Header
            header = customtkinter.CTkLabel(self.tasks_container, 
                width=400, height=20, text=f'LEVEL {level}', 
                font=self.main.get_font(size=15, bold=True))
            header.pack()

            seperator = customtkinter.CTkFrame(self.tasks_container, 
                width=400, height=2, fg_color="white")
            seperator.pack(pady=2)
        ...
        def load_widgets():
            ...
            self.tasks_container.place(relx=0.5,rely=0.5,
                anchor=customtkinter.CENTER
            )
    ...
    def load_completed_tasks(self):
        ...
        header = customtkinter.CTkLabel(self.tasks_container, 
            width=400, height=20, text=f'COMPLETED', 
            font=self.main.get_font(size=15, bold=True))
        header.pack()
    ...
    def task_complete_activated(self, unique_id: str = None):
        ...
        # Some early return statements and notificiations
        # to ensure task name and description meet requirements
        if len(task_name) == 0:
        ...
        if not check_desc_length(task_description):
            self.display_task_status("App_InvalidDescriptionLength",
                msg="Task description",
                rely=0.8
            )
            return
    
        # Checks for symbols and non-accepted characters
        if not is_valid(task_name):
            self.display_task_status("App_InvalidInput", 
                type="Name",
                rely=0.8
            )
            return
        ...
        if not unique_id:
            ...
            timer = Timer(date=self.task_date.cget('text'), 
                time=self.time.get(),
                name=task_name, fn=self.main.task_due_notification)
            timer.start()
            due_date_timers[unique_id] = timer

            if any(sub_dic.get('name').lower() == task_name.lower() 
                for sub_dic in list_data['tasks'].values()
            ):
                self.display_task_status("App_InvalidName", 
                    type="task", msg=task_name, rely=0.8)
                return
        else:
...
class List(customtkinter.CTkFrame):
    ...
    def load_list_frame(self):
        ...
        self.lists_frame = customtkinter.CTkFrame(self.main_frame, 
            width=500, height=400, fg_color="#383736")
        ...
        if  (
            ...
            self.welcome_label = customtkinter.CTkLabel(self.main_frame, 
        ...
        customtkinter.CTkButton(self.lists_frame, text="CREATE LIST",
            width=200, height=35,
        ...
        if data['lists']:
            self.lists_container = customtkinter.CTkScrollableFrame(
                self.lists_frame, width=400, height=290,
                scrollbar_button_color="#2d2c2c", fg_color="transparent")
            
            self.lists_container.place(relx=0.5,rely=0.45,
                anchor=customtkinter.CENTER)
            
            self.load_saved_lists(data)
        else:
            customtkinter.CTkLabel(self.lists_frame, 
                text="You have no lists \ncreate one to start setting tasks",
                font=self.main.get_font()).place(relx=0.5,rely=0.45,
                    anchor=customtkinter.CENTER)
    ...
    def load_create_list_menu(self):
        ...
        self.create_list_frame = customtkinter.CTkFrame(self.main_frame, 
            width=500, height=300, fg_color="#383736")
        self.create_list_frame.place(relx=0.5,rely=0.5,
            anchor=customtkinter.CENTER)
        ...
        customtkinter.CTkButton(self.create_list_frame, text="X", 
            width=35, corner_radius=0,
            font=self.main.get_font(size=22, bold=True),
            fg_color="red", hover_color="dark red",
            command=lambda: self.reload_list_frame(self.create_list_frame)
        ).place(relx=0.99,rely=0.07,anchor='e')

        self.main.seperator(self.create_list_frame)

        create_list_name = customtkinter.CTkEntry(self.create_list_frame, 
            placeholder_text="NAME", corner_radius=0,
            width=400, border_width=0, fg_color="#D9D9D9", 
        ...
        customtkinter.CTkButton(self.create_list_frame, text="COMPLETE", 
            text_color="white", width=200, font=self.main.get_font(), 
            command=lambda:
    ...
    def display_list_status(self, 
        ...
        self.status_error = customtkinter.CTkLabel(self.create_list_frame, 
            text=Constants.DisplayErrors[status_error].format(
                msg=msg, type=type),
            font=self.main.get_font(), fg_color="transparent")
    ...
    def create_list_complete_activated(self, list_name: str):
        ...
        if list_name.lower() in data['lists']:
            self.display_list_status("App_InvalidName", 
                type="list", 
                msg=list_name
            )
            return
    ...
    def load_saved_lists(self, data):
        for list_name, list_data in data['lists'].items():
            ...
            list_item = customtkinter.CTkFrame(self.lists_container, 
                width=400, height=100, fg_color="#5c5b5a")
            ...
            customtkinter.CTkLabel(list_item, 
                text=isinstance(unfinished_tasks_count, dict) 
            ...
            customtkinter.CTkButton(list_item, 
                text="TASKS", width=70, height=20, corner_radius=0,
                fg_color="#ECB528", hover_color="#C8940F", 
                text_color="white", command=task.load_tasks_frame,
                font=self.main.get_font(size=12, bold=True)
            ).place(relx=0.78, rely=0.15,anchor="e")

            customtkinter.CTkButton(list_item,
                text="DELETE", width=70, height=20, corner_radius=0,
                fg_color="#D74E4E", hover_color="#A92727", text_color="white", 
                command=lambda 
                    list_name=list_name: self.list_delete_activated(list_name),
                font=self.main.get_font(size=12, bold=True)
...
class Notification(customtkinter.CTk):
    ...
    def __init__(self, root):
        ...
        self.notification_frame = customtkinter.CTkFrame(root, 
            width=100, height=50,
...
class App:
    def __init__(self, main, username: str):
        ...
        self.load_fonts()
        self.list = List(master=self.main.root, main=self,
            data=self.user_data, username=username)
        
        self.notification = Notification(self.main.root)
        self.notification_ready.set()

        customtkinter.CTkButton(self.main.root, text="FAQs",
            width=100, height=25, command=self.open_faqs
        ).place(relx=0.92,rely=0.88,anchor=customtkinter.CENTER)

        customtkinter.CTkButton(self.main.root, text="Log out",
            width=100, height=25, command=self.logout
    ...
    def open_faqs(self):
        ...
        def load_state_colour_meaning():
            frame = customtkinter.CTkFrame(self.main.root,
                width=250, height=350)
            frame.pack_propagate(False)
            frame.place(relx=0.37, rely=0.5, anchor=customtkinter.CENTER)

            colours = customtkinter.CTkFrame(frame,
            ...
            def create_color_state(index: int):
                customtkinter.CTkFrame(colours,
                    width=20, height=20,
                    fg_color=STATE_COLOURS[index], bg_color=STATE_COLOURS[index],
                ).grid(row=index, pady=20, padx=10)

            info = customtkinter.CTkFrame(frame,
                width=150, height=300,
            )
            info.place(relx=0.6, rely=0.55, anchor=customtkinter.CENTER)

            def create_color_state_info(index: int):
                customtkinter.CTkLabel(info, 
        ...
        title = customtkinter.CTkLabel(self.main.root, 
            text="FAQs", 
            font=customtkinter.CTkFont(family="Arial", 
                size=25, weight="normal"
            )
        ...
        self.faq_back = customtkinter.CTkButton(self.main.root,
